...
### - **Encrypt/Decrypt**
- Moteur Vault qui permettent de chiffrer et déchiffrer de la données
...
`[RAPPEL]` L'application à accès à la cyphertext datakey via :
...
-1.3. Créer une `policy` avec le moteur `transit` puis path `encrypt` et `decrypt` (Penser à créer une arborescence claire afin d'avoir un vision claire de Vault)
- Ici on utilise l'arborescence de `VaultTraining`
```
/home/sednal/
  │ 
  └── vault-config/
...
-2.3. Variabilisé la donnée chiffrée avec la version 3 de la clé
```
ENCRYPTED_SECRET=$(vault write -f -field="ciphertext" transit/encrypt/test1 plaintext=$(echo "test pour rewrap" | base64))
```
