...
`=>` Ouvrir la console et créer password pour `postgres` (étant donné que nous sommes dans une phase de test, et que l'on à besoin d'un super utilisateur)  
```
psql
...
`[NOTE]` Dans un pemier temps utilisation d'un role dynamique 
...
`[NOTE]` Création + Utilisation role Static
