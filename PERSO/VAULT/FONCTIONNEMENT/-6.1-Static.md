...
- Ici le seul chemin par défaut est `/secret`
...
- `Commandes` (Non exhaustives, à titre d'exemple...)
...
```
`[NOTE]` Le `ttl` est à titre indicatif pour l'utilisateur
...
- 1.5 Ajouter un secret dans test ([RAPPEL] cela écrase la donnée précédente)
```
...
`[NOTE]` Si l'on refait un get sur les metadata, toutes les version ainsi que les informations les concernant seront présentes.
...
`[NOTE]` Ici le token accessor est utile pour notamment voir si le secret peux encore être `unwrap` sont ttl, etc..
